...
void decompress_exomizer(u8* from, u8* to)
{
        __asm
                ...
                exo_get4bits:   call    exo_getbit      ;get one bit
                                ...
                                push    hl
                                ld      hl, #1
                                .db #0xFD, #0x71, 41  ;; ld      (iy + #41), c
                exo_setbit:     dec     c
                                jr      nz, exo_setbit-1 ;jump to add hl,hl instruction
                                .db #0xFD, #0x73, 0xF5  ;; ld      (iy-#11), e
                                .db #0xFD, #0x72, 93  ;; ld      (iy+#93), d
                                add     hl, de
                                ex      de, hl
                                inc     iy
                                pop     hl
                ...
                exo_getpair:    add     iy, bc
                                ld      e, d
                                .db #0xFD, #0x46, #41  ;; ld      b, (iy+#41)
                                call    exo_getbits
                                ex      de, hl
                                .db #0xFD, #0x4E, #0xF5  ;; ld      c, (iy-#11)
                                .db #0xFD, #0x46, #93  ;; ld      b, (iy+#93)
